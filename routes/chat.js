...
const CONTENT_FILE = "./data/siteContent.json";
...
async function ensureVectors() {
  ...
  if (loading) {
    while (!vectors) await new Promise(r => setTimeout(r, 500));
    return;
  }
  ...
  for (const text of content) {
    ...
    result.push({ content: text, embedding: Array.from(emb.data) });
  }

  fs.writeFileSync(VECTOR_FILE, JSON.stringify(result));
  vectors = result;
  loading = false;
